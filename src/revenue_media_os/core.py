...
class Telemetry:
    ...
    def record(self, publication_id, keyword_id, rank=None, traffic=None, revenue=None, provider_status="NOT_CONFIGURED", checkpoint="custom"):
        if checkpoint not in self.checkpoints: raise ValueError(f"unsupported telemetry checkpoint: {checkpoint}")
        t=now(); self.db.conn.execute("INSERT INTO rank_history(publication_id,keyword_id,rank,checkpoint,captured_at,provider_status) VALUES(?,?,?,?,?,?)",(publication_id,keyword_id,rank,checkpoint,t,provider_status)); traffic=traffic or {}; self.db.conn.execute("INSERT INTO traffic_metrics(publication_id,checkpoint,captured_at,impression,click,ctr,google_traffic,naver_traffic,sns_traffic,direct_traffic,engagement_time,page_views,provider_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(publication_id,checkpoint,t,*[traffic.get(k) for k in ('impression','click','ctr','google_traffic','naver_traffic','sns_traffic','direct_traffic','engagement_time','page_views')],provider_status)); revenue=revenue or {}; self.db.conn.execute("INSERT INTO revenue_metrics(publication_id,checkpoint,captured_at,adsense_revenue,adpost_revenue,rpm,provider_status) VALUES(?,?,?,?,?,?,?)",(publication_id,checkpoint,t,revenue.get('adsense_revenue'),revenue.get('adpost_revenue'),revenue.get('rpm'),provider_status)); self.db.conn.commit(); self.db.add_audit("telemetry.record","publication",publication_id,provider_status,{"rank":rank,"checkpoint":checkpoint});
...
def period_report(db, period):
    if period not in {"weekly","monthly"}: raise ValueError("period must be weekly or monthly")
    rows=db.conn.execute("SELECT o.decision, count(*) count, coalesce(sum(rm.adsense_revenue),0)+coalesce(sum(rm.adpost_revenue),0) revenue FROM opportunities o LEFT JOIN content_plans cp ON cp.opportunity_id=o.id LEFT JOIN contents c ON c.plan_id=cp.id LEFT JOIN publications p ON p.content_id=c.id LEFT JOIN revenue_metrics rm ON rm.publication_id=p.id GROUP BY o.decision ORDER BY revenue DESC").fetchall()
    return {"period":period,"by_content_type":[dict(r) for r in rows],"top_sites":[dict(r) for r in db.conn.execute("SELECT s.id site_id,s.topic,s.country,s.platform,count(p.id) publications FROM sites s LEFT JOIN publications p ON p.site_id=s.id GROUP BY s.id ORDER BY publications DESC LIMIT 10")],"revenue_status":"NOT_CONFIGURED","ai_cost":db.conn.execute("SELECT coalesce(sum(amount),0) FROM cost_metrics").fetchone()[0]}
