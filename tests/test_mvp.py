...
class MvpTest(unittest.TestCase):
    ...
    def test_schema_versioning(self):
        self.assertEqual(self.db.conn.execute("SELECT max(version) FROM schema_version").fetchone()[0],4); self.assertGreaterEqual(self.db.conn.execute("SELECT count(*) FROM migration_history").fetchone()[0],1)
