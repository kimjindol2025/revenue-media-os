...
INSERT OR IGNORE INTO system_config(key,value) VALUES ('fast_signal_min_velocity','20');
INSERT OR IGNORE INTO system_config(key,value) VALUES ('fast_signal_min_acceleration','0');
...
CREATE TABLE IF NOT EXISTS signals (
  ...
  status TEXT NOT NULL,
  is_fast_candidate INTEGER NOT NULL DEFAULT 0,
  observation_id INTEGER REFERENCES signal_observations(id),
  idempotency_key TEXT UNIQUE
);
...
CREATE TABLE IF NOT EXISTS opportunities (
  ...
  decision_mode TEXT NOT NULL DEFAULT 'FIXTURE',
  score REAL NOT NULL,
  decision_reason TEXT NOT NULL,
  score_components TEXT NOT NULL,
  input_statuses TEXT NOT NULL DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS sites (
  ...
  authority_tags TEXT NOT NULL,
  publisher_type TEXT NOT NULL,
  ads_type TEXT,
  ads_account_ref TEXT,
  search_console_ref TEXT,
  analytics_ref TEXT,
  average_rpm REAL NOT NULL DEFAULT 0,
  average_revenue REAL NOT NULL DEFAULT 0,
  health_status TEXT NOT NULL DEFAULT 'UNKNOWN',
  policy_status TEXT NOT NULL DEFAULT 'UNKNOWN'
);

CREATE TABLE IF NOT EXISTS site_selections (
  id INTEGER PRIMARY KEY,
  opportunity_id INTEGER REFERENCES opportunities(id),
  selected_site_id INTEGER NOT NULL REFERENCES sites(id),
  keyword TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS rank_history (
  ...
  keyword_id INTEGER NOT NULL REFERENCES keywords(id),
  rank INTEGER,
  checkpoint TEXT NOT NULL DEFAULT 'custom',
  captured_at TEXT NOT NULL,
  provider_status TEXT NOT NULL,
  idempotency_key TEXT UNIQUE
);

-- Legacy Stage 1/1.5 traffic table retained for migration compatibility only.
...
CREATE TABLE IF NOT EXISTS cost_metrics (
  ...
  status TEXT NOT NULL,
  idempotency_key TEXT UNIQUE,
  CHECK ((opportunity_id IS NOT NULL) + (content_id IS NOT NULL) + (publication_id IS NOT NULL) <= 1)
);
...
CREATE INDEX IF NOT EXISTS idx_search_pub_time ON search_metrics(publication_id,captured_at);
CREATE INDEX IF NOT EXISTS idx_analytics_pub_time ON analytics_metrics(publication_id,captured_at);
